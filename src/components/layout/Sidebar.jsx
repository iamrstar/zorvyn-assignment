...
import {
  Wallet, ChevronLeft, ChevronRight, LogOut, X,
  LayoutDashboard, Receipt, Lightbulb
} from 'lucide-react';

const Sidebar = ({ activeTab, setActiveTab, isOpen, setIsOpen, mobileOpen, isMobile }) => {
  const links = [
  ...
  return (
    <aside style={{
      ...
      width: mobileOpen ? '280px' : (visible ? '260px' : '72px'),
      ...
      <div style={{ padding: visible ? '24px 20px' : '24px 0', transition: 'padding 0.3s ease' }}>
        <div style={{
          ...
          {mobileOpen && (
            <button onClick={() => setActiveTab(activeTab)} style={{ color: 'var(--text-muted)', padding: '4px' }}>
              <X size={20} />
            </button>
          )}
        </div>
      ...
      <div style={{ padding: visible ? '16px 12px' : '16px 8px', borderTop: '1px solid var(--border)' }}>
        {visible && (
          <div style={{
            ...
            <p style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--primary)', marginBottom: '4px' }}>Pro Tip</p>
